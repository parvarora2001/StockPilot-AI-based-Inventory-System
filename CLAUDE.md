...
2. **`forecaster.py`** — `forecast_demand(product_id, forecast_days)` returns history + prediction + summary. Model is **Holt-Winters additive** (level + trend + 7-day seasonality) via `statsmodels.tsa.holtwinters.ExponentialSmoothing`. Confidence band is `±1.5 × std(residuals)`. Trend is bucketed by comparing the smoothed mean of the last 14 days to the first 14 days (±5%). 90-day history window is hardcoded. **Falls back** to a 14-day moving average if HW fails to converge (rare — happens on near-constant series); `summary.method` reports which was used. Also computes a residual-based **anomaly flag** (`summary.anomaly` is `True` when any of the last 7 days has |z-score| > 2.5).
